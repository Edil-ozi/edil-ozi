...
const movable =[
  ...
  {movable: false, velocity: -5},
]

export default function ScrollVelocityParallaxDemo() {
  return (
    <div className="w-full">
      <h2 className="text-center text-3xl">Scroll To See Effect</h2>
      <div className="flex flex-col space-y-5 py-10">
        {movable.map(({movable, velocity}, index) =>
          <ScrollVelocityParallax key={index} velocity={velocity} movable={movable}>
            {images.map(({title, thumbnail}) =>
              <div key={title} className="relative h-[22rem] w-[25rem]">
                <Image
