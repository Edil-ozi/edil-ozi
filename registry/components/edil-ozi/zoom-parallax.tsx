...
const ZoomParallax: FC<Props> = ({ classes }) => {
  ...
  return (
    <section ref={container} className={cn("relative h-[200vh] w-full z-20", classes)}>
          ...
          return (
            <motion.div key={index} style={{ scale }} className="w-full h-full top-0 absolute flex items-center justify-center">
              <div className={classes}>
                <Image src={src} fill objectFit="cover" alt="image" quality="50" />
              </div>
            </motion.div>
          );
        })}
      </div>
      <div className="leading-[0] h-[100vh]"></div>
