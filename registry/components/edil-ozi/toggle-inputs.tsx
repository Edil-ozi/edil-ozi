  ...
  const toggle_inputs = [
    {
      ...
      component: (
        <label className="text-[17px] relative inline-block w-[3.7em] h-[1.8em] ">
          <input tabIndex={-1} type="checkbox" className="hidden opacity-0 w-0 h-0 [&:checked+span:before]:bg-green-500/70 dark:[&:checked+span:before]:bg-green-500 [&:checked+span:before]:translate-x-[1.9em] [&:checked+span]:bg-green-300 dark:[&:checked+span]:bg-green-900" />
...

    {
      ...
      component: (
        <input type="checkbox" className="relative h-6 w-12 appearance-none bg-stone-300 transition duration-300 ease rounded-full checked:bg-orange-600 before:absolute before:content-[''] before:left-[calc(1.5em_-_1.6em)] before:top-[calc(1.5em_-_1.6em)] before:block before:h-[1.6em] before:w-[1.6em] before:cursor-pointer before:border before:bg-white before:transition-all before:duration-300 before:ease before:rounded-full before:border-solid before:border-stone-400 hover:before:shadow-[0_0_0px_8px_rgba(0,0,0,0.15)] checked:hover:before:shadow-[0_0_0px_8px_rgba(236,72,72,0.15)] checked:before:translate-x-full checked:before:border-orange-600" />
      ),
    },
  ];
